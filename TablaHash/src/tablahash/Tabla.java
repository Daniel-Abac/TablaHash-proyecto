...
public class Tabla {
    ...
    */
    ...
    public boolean Insertar(String nombre, String anio,String genero, int id){
        boolean Insercion_Correctamente = false;
        if(FuncionHash == 1){ // Funcion de division 
             int posicion = FuncionDivision(id); // se encuentra la posicion a insertar 
             Insercion_Correctamente = InsertarValor(nombre, anio, genero, id, posicion);// se manda a insertar el valor
        }
        else{// funcion de multiplicacion
        
         int posicion = FuncionMultiplicar(id);
         Insercion_Correctamente = InsertarValor(nombre, anio, genero, id, posicion);// se manda a insertar el valor en la tabla 
                 
        }
       return Insercion_Correctamente; 
    }
    
    public boolean  InsertarValor(String nombre, String anio, String genero, int id, int posicion){
        ...
        if(TablaHash[posicion] != null){ // si en la posicion ya hay una lista 
            ...
            //si el numero telefonico ya existe no inserta en dado caso no existe lo inserta 
                if(!lista_auxiliar.Buscar(id)){ //si el numero no existe 
                    lista_auxiliar.InsertarFondo(nombre, anio, genero, id); // se insertar en la lista 
            ...
            nueva_lista.InsertarFondo(nombre, anio, genero, id);// se inserta el nuevo numero 
  ...
  //Mostrar la tabla   
    ...
    //*Funcion de division 
    private int FuncionDivision(int id){
        int posicion = 0;
        posicion = id % numM; // se encuentra la posicion a insertar 
        cadena = "Posicion en donde se inserto: "+posicion;
        return posicion;
    }
    //Funcion de multiplicacion
    private int FuncionMultiplicar(int id){
        int posicion = 0;
         BigDecimal multi = BigDecimal.valueOf(id);// se coloca el telefono en una variable que soporte un numero decimal granded 
 ...
 //Buscar valor. 
    public boolean Buscar(int id){
        ...
        int posicion;
        if(FuncionHash == 1){
            posicion = FuncionDivision(id);
            if(TablaHash[posicion]!=null){
                existe = TablaHash[posicion].Buscar(id);
                cadena = TablaHash[posicion].getCadena();
            }
        }
        else{
            posicion = FuncionMultiplicar(id);
            if(TablaHash[posicion]!=null){
                existe = TablaHash[posicion].Buscar(id);
                cadena = TablaHash[posicion].getCadena();
            }
  ...
  public boolean Eliminar(int id){
      ...
      int posicion;
      if(FuncionHash == 1){
            posicion = FuncionDivision(id);// buscamos la posicion donde deberia estar 
            ...
            if(TablaHash[posicion]!=null ){// la lista tiene que ser distinta de nulo
                if(lista_aux.Buscar(id)){//el valor tiene que existir en la lista 
               lista_aux.Eliminar(id);// eliminamos el valor de la lista 
